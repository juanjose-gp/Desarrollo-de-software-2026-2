...
public class pagoService {
    ...
    public void crearPago(Pago pago) {
        if (pago == null || pago.getFactura() == null || pago.getValor() <= 0) {
            System.out.println("Los datos del pago no son validos.");
            return;
        }

        if (buscarPagoPorId(pago.getId()) != null) {
            System.out.println("Ya existe un pago con ese ID.");
            return;
        }
    ...
    // UPDATE
    public boolean actualizarPago(int id, double nuevoValor) {

        Pago pago = buscarPagoPorId(id);

        if (pago != null) {
            pago.setValor(nuevoValor);
            return true;
        }
    ...
    // Mostrar pagos de un cliente
    public void listarPagosCliente(int idCliente) {

        List<Pago> resultado =
                obtenerPagosPorCliente(idCliente);
